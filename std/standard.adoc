...
Add a row to the top of the table (directly below the header) each time a new version of the Standard is published (i.e., newer versions appear at the top of the table; first published version appears in the bottom row)

NOTE: 
 Approval Date:  this will automatically be pulled from the VARIABLES at the top. If you are writing an updated version of a std, the approval date must be hard coded for the earlier version(s), and the variable for approval date must be used only for the most recent version. 
...
The description box must not be left blank, but it should be brief. 
   New stds:  Why was there a need for this std?
   New versions of existing stds:  Why was there a need for an updated version?
...
NUMBERED SECTIONS:
  This is the point at which document numbering begins.
   ...
   Make sure to check where information needs to be added (e.g., the parenthetical section of the PURPOSE)
////
