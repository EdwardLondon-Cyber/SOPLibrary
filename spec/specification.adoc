...
The table below provides a quick view of the *_critical_* security configuration settings. All *_critical_* configurations *_SHALL NOT_* be eligible for any modifications through the application of an RBD. The *_critical_* settings in the table are linked to a detailed explanation of the configuration setting in the subsequent section of this Specification.
...
This section contains all security configuration settings that are [.underline]#required# for Agency compliance, as well as a description for each setting. 

[.underline]#Required# controls *_SHALL_* be adhered to, as they are monitored, scored, and reported by the Agency. 

Settings that are [.underline]#required# *_MAY_*, in limited and justified instances, be amended through the proper application of an RBD. To pursue an RBD, see <<Tailoring>> in this Specification.
...
This section contains all [.underline]#recommended# settings. Settings that are classified as [.underline]#recommended# are not required, but they are deemed highly beneficial by ASCS. These settings are expected to be implemented, unless there is a justifiable cause not to. However, [.underline]#recommended# settings that are not implemented will not be scored as deviation from compliance.
////
